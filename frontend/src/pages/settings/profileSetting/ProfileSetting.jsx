...
const ProfileSetting = ({ setOpenBar }) => {
  ...
  const formSchema = z.object({
    ...
    brithDate: z.string().regex(/\d{4}-\d{2}-\d{2}$/, "Birth Date is required"),
  ...
  async function onSubmit(values) {
    if (
      ...
      toast.info(
        "Modify the information you want to update before submitting."
      );
      return;
    }

    if (image) {
      await uploadImage(image, values, setImageUrl, token, signIn);
      return;
    }
    try {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-7">
            <div className="flex items-center justify-center pb-4 relative">
              ...
              />
              <label
                className="absolute bottom-2 mr-20
